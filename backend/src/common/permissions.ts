...
export const PERMISSION_CATALOGUE: Array<{
  ...
  { key: PERMISSIONS.TENANT_MANAGE, group: 'Tenants', description: 'Create and edit tenants and stays' },
  { key: PERMISSIONS.TENANT_ARCHIVE, group: 'Tenants', description: 'Archive or erase tenant records' },
  ...
  { key: PERMISSIONS.PAYMENT_RECORD, group: 'Money', description: 'Record payments' },
...
/** Seed roles. Editable afterwards — these are starting points, not rules. */
export const DEFAULT_ROLES: Array<{
...
}> = [
  {
    key: 'owner',
    name: 'Owner',
    description: 'Full access to everything including settings and audit',
    permissions: 'ALL',
  },
  {
    key: 'manager',
    name: 'Manager',
    description: 'Day-to-day operations: tenants, rooms, money, reports',
    permissions: [
      PERMISSIONS.BRANCH_VIEW, PERMISSIONS.ROOM_VIEW, PERMISSIONS.ROOM_MANAGE,
      PERMISSIONS.TENANT_VIEW, PERMISSIONS.TENANT_MANAGE, PERMISSIONS.TENANT_ASSIGN,
      PERMISSIONS.TENANT_VACATE, PERMISSIONS.DOCUMENT_VIEW, PERMISSIONS.DOCUMENT_MANAGE,
      PERMISSIONS.DOCUMENT_VERIFY, PERMISSIONS.PRICING_VIEW, PERMISSIONS.INVOICE_VIEW,
      PERMISSIONS.INVOICE_MANAGE, PERMISSIONS.PAYMENT_VIEW, PERMISSIONS.PAYMENT_RECORD,
      PERMISSIONS.DEPOSIT_VIEW, PERMISSIONS.DEPOSIT_MANAGE, PERMISSIONS.SETTLEMENT_VIEW,
      PERMISSIONS.SETTLEMENT_MANAGE, PERMISSIONS.EB_VIEW, PERMISSIONS.EB_MANAGE,
      PERMISSIONS.EXPENSE_VIEW, PERMISSIONS.EXPENSE_MANAGE, PERMISSIONS.REPORT_VIEW,
      PERMISSIONS.EXPORT_RUN, PERMISSIONS.SETTINGS_VIEW,
    ],
  },
  {
    key: 'caretaker',
    name: 'Caretaker',
    description: 'Front-desk: sees tenants and rooms, records payments and readings',
    permissions: [
      PERMISSIONS.BRANCH_VIEW, PERMISSIONS.ROOM_VIEW, PERMISSIONS.TENANT_VIEW,
      PERMISSIONS.TENANT_MANAGE, PERMISSIONS.DOCUMENT_VIEW, PERMISSIONS.DOCUMENT_MANAGE,
      PERMISSIONS.INVOICE_VIEW, PERMISSIONS.PAYMENT_VIEW, PERMISSIONS.PAYMENT_RECORD,
      PERMISSIONS.EB_VIEW, PERMISSIONS.EB_MANAGE, PERMISSIONS.EXPENSE_VIEW,
    ],
  },
  {
    key: 'viewer',
    ...
    permissions: [
      PERMISSIONS.BRANCH_VIEW, PERMISSIONS.ROOM_VIEW, PERMISSIONS.TENANT_VIEW,
      PERMISSIONS.INVOICE_VIEW, PERMISSIONS.PAYMENT_VIEW, PERMISSIONS.DEPOSIT_VIEW,
      PERMISSIONS.EB_VIEW, PERMISSIONS.EXPENSE_VIEW, PERMISSIONS.REPORT_VIEW,
    ],
  },
];
